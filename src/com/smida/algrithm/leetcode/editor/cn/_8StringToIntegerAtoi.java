...
public class _8StringToIntegerAtoi {
    ...
    class Solution {
        ...
        //2、判断开头的数字是不是0
        //3、判断数字，此处要防止进位后超出限制
        //4、最后还要判断一次限制，防止进位后加上个位数后超出限制
        public int myAtoi(String s) {
            s = s.trim();
            long res = 0;
            Boolean bePositive = true;
            for (int i = 0; i < s.length(); i++) {
                if (0 == i && (s.charAt(i) == '+' || s.charAt(i) == '-')) {
                    bePositive = s.charAt(i) == '-' ? false : true;
                    continue;
                }
                if (res == 0 && s.charAt(i) == 0) {
                    continue;
                }
                ...
                if (bePositive && res * 1.0 / Integer.MAX_VALUE > 0.1) {
                    return Integer.MAX_VALUE;
                } else if (!bePositive && res * 1.0 / Integer.MIN_VALUE > 0.1) {
                    return Integer.MIN_VALUE;
                }
                res = res * 10 + (bePositive ? cur : -1 * cur);
            }
            return bePositive ?
